...
final class TerminalSessionState {
  ...
  private var lastReportedTaskStatus: TerminalTaskStatus?
  ...
  var onTaskStatusChanged: ((TerminalTaskStatus) -> Void)?
  var onRunScriptStatusChanged: ((Bool) -> Void)?
  var onCommandPaletteToggle: (() -> Void)?
  var onSetupScriptConsumed: (() -> Void)?

  init(runtime: GhosttyRuntime, session: TerminalSession, runSetupScript: Bool = false) {
  ...
  func ensureInitialTab(focusing: Bool) {
    ...
    if tabManager.tabs.isEmpty {
      _ = createTab(focusing: focusing, setupScript: nil)
    }
  ...
  func createTab(
    ...
    let title = "\(session.name) \(nextTabIndex())"
  ...
  func dismissNotification(_ notificationID: TerminalNotification.ID) {
    let previousHasUnseen = hasUnseenNotification
